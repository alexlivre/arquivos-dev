...
- Sugerir a manutenção dos arquivos estáticos em diretórios organizados (ex: `assets/js/` e `assets/css/`).
...
### 4) Fontes — Evitar bloqueio, FOUT/FOIT e Latência

**A. Auto-hospedar e Formato WOFF2**
- Hospedar fontes localmente (ex: em `assets/fonts/`) elimina a latência de DNS/conexão com servidores externos.
- **Ação Crítica:** Remover completamente dependências externas no `<head>` (deletar tags `<link>` para `fonts.googleapis.com` ou Typekit).
- Substituir arquivos `.ttf` ou `.otf` pelo formato **`.woff2`**, o que reduz o tamanho do arquivo em ~70%.
...
**C. Preload seletivo**
- Usar `<link rel="preload">` para a fonte mais importante (acima da dobra/Hero Section) para evitar FOIT, mas com cautela — preload de muitas fontes disputa com recursos críticos.
...
### 9) Percepção Visual e UX (Mobile)

- **Theme Color:** Adicionar a meta tag `<meta name="theme-color" content="#HEX">` no `<head>`, utilizando a cor de fundo predominante do site. Isso faz a barra de endereços do navegador mobile combinar com o site, melhorando significativamente a percepção de carregamento do usuário.
...
- **Fontes**: estratégia de formato `.woff2`, deleção de links externos, `font-display` e preloads.
- **UX**: Sugestão de cor exata para a tag `theme-color`.
...
- **Ação Obrigatória Final:** Sempre instruir o usuário a rodar o comando de build de produção pertinente à stack (ex: `npm run build`) para garantir que os arquivos finais sejam efetivamente minificados.
...
- Não manter links no HTML para serviços de fontes externas se a decisão foi auto-hospedar.
